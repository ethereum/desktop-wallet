...
//!
//! Must stay named `main.rs`: cargo builds `tests/*.rs` and `tests/*/main.rs` and nothing
//! else, so renaming this file stops every test here from running, silently.
//!
...
/// `m_cost` of 65536 as a postcard varint. The `t_cost` byte follows it inside the password
/// slot's parameter blob, which is easier to locate by this marker than by a fixed offset.
const M_COST_VARINT: [u8; 3] = [0x80, 0x80, 0x04];
...
async fn header_key_derivation_parameters_are_not_trusted() {
    ...
    // postcard: magic[8] | version[1] | slots, each kind | params | wrapped, with the
    // Argon2id parameters inside params as m_cost | t_cost | p_cost | salt.
    // Raising t_cost in place multiplies the work unlock would perform.
    let t_cost_offset = bytes
        .windows(M_COST_VARINT.len())
        .position(|window| window == M_COST_VARINT)
        .expect("m_cost varint not found; header layout changed")
        + M_COST_VARINT.len();
